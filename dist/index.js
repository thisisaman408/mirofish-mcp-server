...
// ========================== HEALTH ==========================
server.registerTool("mirofish_health_check", {
    title: "Health Check",
    description: "Check if the MiroFish backend is running and healthy. Returns service status.",
    inputSchema: {},
    annotations: {
        readOnlyHint: true,
...
// ========================== PROJECT MANAGEMENT ==========================
server.registerTool("mirofish_list_projects", {
    title: "List Projects",
    description: "List all MiroFish projects. Each project contains seed documents and ontology definitions for simulation.",
    inputSchema: {
        limit: z
            .number()
            .int()
...
server.registerTool("mirofish_get_project", {
    title: "Get Project",
    description: "Get details of a specific MiroFish project by its ID, including ontology, files, status, and graph info.",
    inputSchema: {
...
server.registerTool("mirofish_delete_project", {
...
server.registerTool("mirofish_reset_project", {
    title: "Reset Project",
    description: "Reset a project's state so its knowledge graph can be rebuilt. Keeps the ontology but clears the graph.",
    inputSchema: {
...
server.registerTool("mirofish_generate_ontology", {
    ...
    inputSchema: {
        ...
        document_text: z
            .string()
            .optional()
            .describe("If you cannot upload files, provide the raw document text here. The backend will need file uploads for the actual endpoint — this is a convenience description."),
    },
    annotations: {
        readOnlyHint: false,
        destructiveHint: false,
...
server.registerTool("mirofish_build_graph", {
    title: "Build Knowledge Graph",
    description: "Build a knowledge graph from a project's documents and ontology using Zep. This is an async operation — returns a task_id to poll with mirofish_get_task.",
    inputSchema: {
        project_id: z
            .string()
...
server.registerTool("mirofish_get_graph_data", {
    title: "Get Graph Data",
    description: "Retrieve the full knowledge graph data (nodes and edges) for a given graph ID.",
    inputSchema: {
...
server.registerTool("mirofish_delete_graph", {
    title: "Delete Graph",
...
// ========================== TASKS ==========================
server.registerTool("mirofish_get_task", {
    title: "Get Task Status",
    description: "Check the status and progress of an async task (graph building, simulation preparation, report generation, etc.).",
    inputSchema: {
...
server.registerTool("mirofish_list_tasks", {
    title: "List Tasks",
...
// ========================== SIMULATION - ENTITIES ==========================
server.registerTool("mirofish_get_entities", {
    title: "Get Graph Entities",
    description: "Get all filtered entities from a knowledge graph. Only returns nodes matching predefined entity types.",
    inputSchema: {
...
server.registerTool("mirofish_get_entity_detail", {
    ...
    const data = await api(`/api/simulation/entities/${graph_id}/${entity_uuid}`);
    return { content: [{ type: "text", text: jsonText(data) }] };
});
server.registerTool("mirofish_get_entities_by_type", {
    ...
    const data = await api(`/api/simulation/entities/${graph_id}/by-type/${entity_type}`, { query: { enrich } });
    return { content: [{ type: "text", text: jsonText(data) }] };
});
// ========================== SIMULATION - LIFECYCLE ==========================
server.registerTool("mirofish_create_simulation", {
    title: "Create Simulation",
    description: "Create a new multi-agent simulation from a project. Requires a completed knowledge graph.",
    inputSchema: {
...
server.registerTool("mirofish_prepare_simulation", {
    title: "Prepare Simulation",
    description: "Prepare a simulation environment — reads entities from the graph, generates agent profiles via LLM, and creates simulation config. Async — returns task_id.",
    inputSchema: {
...
server.registerTool("mirofish_prepare_status", {
...
server.registerTool("mirofish_start_simulation", {
    title: "Start Simulation",
    description: "Start running a prepared simulation. Agents will begin interacting in the simulated environment.",
    inputSchema: {
...
server.registerTool("mirofish_stop_simulation", {
...
server.registerTool("mirofish_get_simulation", {
...
server.registerTool("mirofish_list_simulations", {
...
server.registerTool("mirofish_simulation_run_status", {
    title: "Get Simulation Run Status",
    description: "Get the real-time running status of a simulation including process state.",
    inputSchema: {
        simulation_id: z.string().describe("The simulation ID"),
    },
    annotations: {
...
server.registerTool("mirofish_simulation_run_status_detail", {
    title: "Get Detailed Run Status",
    description: "Get detailed running status of a simulation including round progress and agent activity.",
    inputSchema: {
        simulation_id: z.string().describe("The simulation ID"),
    },
    annotations: {
    ...
    const data = await api(`/api/simulation/${simulation_id}/run-status/detail`);
    return { content: [{ type: "text", text: jsonText(data) }] };
});
server.registerTool("mirofish_get_simulation_profiles", {
...
server.registerTool("mirofish_get_simulation_config", {
...
// ========================== SIMULATION - RESULTS ==========================
server.registerTool("mirofish_get_simulation_actions", {
    title: "Get Simulation Actions",
    description: "Get the action log of a simulation — all agent actions taken during the run.",
    inputSchema: {
        simulation_id: z.string().describe("The simulation ID"),
    },
    annotations: {
...
server.registerTool("mirofish_get_simulation_timeline", {
...
server.registerTool("mirofish_get_agent_stats", {
...
server.registerTool("mirofish_get_simulation_posts", {
    title: "Get Simulation Posts",
    description: "Get social media posts generated by agents during the simulation (Twitter/Reddit style).",
    inputSchema: {
        simulation_id: z.string().describe("The simulation ID"),
    },
    annotations: {
...
server.registerTool("mirofish_get_simulation_comments", {
    title: "Get Simulation Comments",
    description: "Get comments/replies generated by agents during the simulation.",
    inputSchema: {
        simulation_id: z.string().describe("The simulation ID"),
    },
    annotations: {
...
server.registerTool("mirofish_interview_agent", {
    title: "Interview Agent",
    description: "Interview (chat with) a specific simulated agent. The agent responds based on its personality, memories, and actions during the simulation.",
    inputSchema: {
        simulation_id: z.string().describe("The simulation ID"),
...
server.registerTool("mirofish_interview_batch", {
    title: "Batch Interview Agents",
    description: "Send the same question to multiple agents at once. Useful for gathering diverse perspectives.",
    inputSchema: {
        simulation_id: z.string().describe("The simulation ID"),
        agent_names: z
            .array(z.string())
...
server.registerTool("mirofish_interview_all", {
    title: "Interview All Agents",
    description: "Send the same question to ALL agents in the simulation. Returns all their responses.",
    inputSchema: {
        simulation_id: z.string().describe("The simulation ID"),
        message: z.string().describe("The question to ask all agents"),
    },
...
server.registerTool("mirofish_interview_history", {
...
server.registerTool("mirofish_env_status", {
...
server.registerTool("mirofish_close_env", {
...
// ========================== REPORTS ==========================
server.registerTool("mirofish_generate_report", {
    title: "Generate Report",
    description: "Generate a prediction/analysis report from a completed simulation. Async — returns task_id to poll.",
    inputSchema: {
        simulation_id: z.string().describe("The simulation ID"),
        force_regenerate: z
            .boolean()
...
server.registerTool("mirofish_report_generate_status", {
...
server.registerTool("mirofish_get_report", {
    title: "Get Report",
    description: "Get a completed report by its ID — includes outline, markdown content, and metadata.",
    inputSchema: {
...
server.registerTool("mirofish_get_report_by_simulation", {
...
server.registerTool("mirofish_list_reports", {
    title: "List Reports",
...
server.registerTool("mirofish_delete_report", {
    title: "Delete Report",
...
server.registerTool("mirofish_get_report_progress", {
    title: "Get Report Progress",
    description: "Get real-time progress of report generation including current section being written.",
    inputSchema: {
        report_id: z.string().describe("The report ID"),
    },
    annotations: {
...
server.registerTool("mirofish_get_report_sections", {
    title: "Get Report Sections",
    description: "Get all generated sections of a report. Can be polled during generation to get sections as they complete.",
    inputSchema: {
        report_id: z.string().describe("The report ID"),
    },
    annotations: {
...
server.registerTool("mirofish_get_report_section", {
    ...
    const data = await api(`/api/report/${report_id}/section/${section_index}`);
    return { content: [{ type: "text", text: jsonText(data) }] };
});
server.registerTool("mirofish_check_report_status", {
    title: "Check Report Status",
    description: "Check if a simulation has a report and its status. Used to determine if interview is unlocked.",
    inputSchema: {
        simulation_id: z.string().describe("The simulation ID"),
    },
    annotations: {
...
// ========================== REPORT - CHAT ==========================
server.registerTool("mirofish_chat_with_report_agent", {
    title: "Chat with Report Agent",
    description: "Have a conversation with the Report Agent about a simulation's results. The agent can search the knowledge graph and use tools to answer your questions.",
    inputSchema: {
        simulation_id: z.string().describe("The simulation ID"),
...
// ========================== REPORT - TOOLS (DEBUG) ==========================
server.registerTool("mirofish_search_graph", {
    title: "Search Knowledge Graph",
    description: "Search the knowledge graph for facts and information. Uses Zep's semantic search.",
    inputSchema: {
...
server.registerTool("mirofish_graph_statistics", {
    title: "Get Graph Statistics",
    description: "Get statistics about a knowledge graph — node counts, edge counts, entity type distribution.",
    inputSchema: {
        graph_id: z.string().describe("The graph ID"),
    },
    annotations: {
...
// ========================== SIMULATION - HISTORY ==========================
server.registerTool("mirofish_simulation_history", {
    title: "Get Simulation History",
    description: "Get the history of all simulations with their run status and results.",
    inputSchema: {
        limit: z.number().int().min(1).max(200).default(50).describe("Max results"),
    },
    annotations: {
